...
import { type TrackMetadata, calculateCompatibilityScore, rankCompatibleTracks, findNextCompatibleTrack, calculateConstantPowerCrossfade, calculatePhaseAlignment } from '../utils/automix';
...
class AutomixEngine {
  ...
  async initialize(libraryTracks: TrackMetadata[]): Promise<void> {
    ...
    console.log('[AutomixEngine] Initialized with', libraryTracks.length, 'tracks');
  }
  ...
  async startAutomix(
    ...
    console.log('[AutomixEngine] Started automix on', masterDeck, 'with track:', initialTrack.title);
  ...
  async triggerTransition(): Promise<boolean> {
    if (!this.state.isActive || !this.state.nextTrack || this.state.isTransitioning) {
      return false;
    }
  ...
  private async ensureTrackAnalysis(track: TrackMetadata): Promise<TrackMetadata> {
    ...
    if (!analyzedTrack.bpm && analyzedTrack.type === 'audio') {
      try {
        ...
        if (audioBuffer) {
          const beatGrid = await this.beatGridService.analyze(audioBuffer, analyzedTrack.id);
          analyzedTrack.bpm = beatGrid.bpm;
          analyzedTrack.duration = audioBuffer.duration;
        }
      } catch (error) {
        console.warn('[AutomixEngine] Failed to analyze BPM for', track.title, error);
      }
    }
    ...
    if (!analyzedTrack.camelot && analyzedTrack.type === 'audio') {
      try {
        const audioBuffer = await this.loadAudioBuffer(analyzedTrack.src);
        if (audioBuffer) {
          const keyResult = await this.keyService.analyzeKey(audioBuffer, analyzedTrack.id);
          if (keyResult.available) {
            analyzedTrack.camelot = keyResult.camelot;
          }
        }
      } catch (error) {
        console.warn('[AutomixEngine] Failed to analyze key for', track.title, error);
      }
    }
  ...
  private async performTransition(): Promise<boolean> {
    ...
    try {
      ...
      const masterTrack = this.libraryTracks.find(t => t.id === this.getCurrentTrackId(masterDeck));
      if (masterTrack) {
        await this.syncAndTransition(masterDeck, slaveDeck, masterTrack, nextTrack);
      }
  ...
  private startCrossfade(masterDeck: 'deckA' | 'deckB', slaveDeck: 'deckA' | 'deckB'): void {
