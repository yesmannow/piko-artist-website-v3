...
export function createSharedControlBlock(): ControlBlockViews {
  ...
  const floats = new Float32Array(sab, FLOAT32_BYTE_OFFSET, FLOAT32_ELEMENT_COUNT);
  
  // Initialize with default values
  initializeControlBlock(ints, floats);
  
  console.log(`[ControlBus] Created SharedArrayBuffer control block (${TOTAL_SAB_BYTES} bytes)`);
...
function initializeControlBlock(ints: Int32Array, floats: Float32Array): void {
  ...
  Atomics.store(ints, INT32_OFFSET.DECK_A_TRANSPORT, DEFAULT_CONTROLS.transport);
  Atomics.store(ints, INT32_OFFSET.DECK_B_TRANSPORT, DEFAULT_CONTROLS.transport);
...
export class ControlBus {
  ...
  setDeckGain(deck: DeckId, gain: number): void {
    const offset = deck === 'A' ? FLOAT32_OFFSET.DECK_A_GAIN : FLOAT32_OFFSET.DECK_B_GAIN;
  ...
  setDeckRate(deck: DeckId, rate: number): void {
    const offset = deck === 'A' ? FLOAT32_OFFSET.DECK_A_RATE : FLOAT32_OFFSET.DECK_B_RATE;
  ...
  setEQ(deck: DeckId, band: EQBand, gain: number): void {
    const baseOffset = deck === 'A' ? FLOAT32_OFFSET.DECK_A_EQ_LOW : FLOAT32_OFFSET.DECK_B_EQ_LOW;
  ...
  setTransport(deck: DeckId, command: TransportCommand): void {
    const offset = deck === 'A' ? INT32_OFFSET.DECK_A_TRANSPORT : INT32_OFFSET.DECK_B_TRANSPORT;
