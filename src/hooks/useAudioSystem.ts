 ...
 */
export function useAudioSystem(config: AudioSystemConfig = {}): AudioSystemState & {
  ...
  const initializeAudio = useCallback(async () => {
    ...
    try {
      ...
      if (workletModules.length > 0) {
        if (debug) {
          console.log('[useAudioSystem] Loading AudioWorklet modules:', workletModules);
        }
        ...
        try {
          ...
          console.error('[useAudioSystem] Failed to load AudioWorklet modules:', workletError);
      ...
      console.error('[useAudioSystem] Failed to initialize audio system:', error);
      setIsReady(false);
    } finally {
...
function playSilentBuffer(audioContext: AudioContext, debug: boolean) {
  try {
    ...
    if (debug) {
      console.log('[useAudioSystem] Silent buffer loop started (iOS audio session keepalive)');
    }
  } catch (error) {
