 ...
 * - Historical trend
 */
export function LatencyMonitor({ audioContext, compact = false }: LatencyMonitorProps) {
  const {
    metrics,
    isBenchmarking,
    isMonitoring,
        ...
        {metrics && (
          <>
            ...
            <span className="text-white/60">
              Grade: <span style={{ color: getStatusColor() }} className="font-bold">{metrics.grade}</span>
            </span>
        ...
        <div className="space-y-3">
          ...
          <div className="flex items-baseline gap-3">
            ...
            <div className="flex items-baseline gap-1">
              <span className="text-4xl font-bold" style={{ color: getStatusColor() }}>
          ...
          <div className="grid grid-cols-2 gap-2 pt-3 border-t border-white/10">
            ...
            <div className="col-span-2">
              ...
              <div className={`font-bold ${metrics.glitchCount > 0 ? 'text-red-400' : 'text-green-400'}`}>
                {metrics.glitchCount}
              </div>
            </div>
          </div>
          ...
          {history.length > 1 && (
            <div className="pt-3 border-t border-white/10">
              <div className="text-white/40 text-xs mb-2">Performance Trend</div>
              <div className="flex items-end gap-1 h-12">
                {history.slice(-10).map((metric, i) => {
                  const height = Math.min(100, (metric.totalLatencyMs / 50) * 100);
