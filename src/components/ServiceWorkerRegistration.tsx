  ...
  useEffect(() => {
    ...
    const purgeServiceWorkersAndCaches = async () => {
      try {
        ...
        try {
          hasPerformedPurge =
            sessionStorage.getItem("sw-cache-purged") === "true";
        } catch (storageError) {
          console.warn(
            "[SW Purge] sessionStorage not available:",
            storageError,
          );
        ...
        if (!hasPerformedPurge) {
          console.log(
            "[SW Purge] Starting service worker and cache cleanup...",
          );
          ...
          for (const registration of registrations) {
            ...
            console.log(
              "[SW Purge] Unregistered service worker:",
              registration.scope,
            );
          }
          ...
          try {
            ...
            console.warn(
              "[SW Purge] Could not set sessionStorage flag:",
              storageError,
            );
          }
        ...
        console.error(
          "[SW Purge] Failed to purge service workers and caches:",
          error,
        );
                ...
                async () => {
                  try {
                    if (
                      ...
                      if (usagePercent > 80) {
                        console.warn(
                          `[SW] Storage usage at ${usagePercent.toFixed(1)}%, triggering cleanup...`,
                        );
