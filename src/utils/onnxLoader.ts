...
export async function loadONNXRuntimeInWorker(): Promise<any> {
  ...
  try {
    ...
    console.warn('[onnxLoader] Dynamic import failed, trying alternative method:', error);

    // For classic workers, we might need to use importScripts
    // But since we're using module workers, this is a fallback
    throw new Error('Failed to load ONNX Runtime. Ensure onnxruntime-web is installed.');
  }
}
...
export async function selectBackend(): Promise<'webgpu' | 'wasm'> {
  try {
    ...
    console.warn('[onnxLoader] WebGPU not available, falling back to WASM:', error);
  }
 ...
 */
export function configureONNXEnvironment(ort: any, backend: 'webgpu' | 'wasm'): void {
  if (backend === 'webgpu') {
