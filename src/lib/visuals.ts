...
async function fetchPexels(query: string, perPage: number, page: number): Promise<VisualImage[]> {
...
async function fetchUnsplash(query: string, perPage: number, page: number): Promise<VisualImage[]> {
...
async function fetchPixabay(query: string, perPage: number, page: number): Promise<VisualImage[]> {
...
export async function searchVisuals({ theme = DEFAULT_QUERY, count = 12, page = 1 }: SearchParams = {}): Promise<VisualImage[]> {
  ...
  const candidates: ReadonlyArray<{
    ...
    { name: "unsplash", fn: fetchUnsplash, hasKey: !!process.env.UNSPLASH_ACCESS_KEY },
    { name: "pixabay", fn: fetchPixabay, hasKey: !!process.env.PIXABAY_API_KEY },
  ] as const;

  const availableProviders = candidates.filter((p): p is typeof candidates[number] => p.hasKey);

  const perProvider = availableProviders.length > 0 ? Math.max(1, Math.floor(count / availableProviders.length)) : count;
  ...
  const images = results.flatMap((r) => (r.status === "fulfilled" ? r.value : [] as VisualImage[]));
  ...
  if (deduped.length < count && availableProviders.length) {
    const fill = await Promise.allSettled(
      availableProviders.map(p => p.fn(query, count - deduped.length, page + 1))
    );
    for (const r of fill) {
