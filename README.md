...
High-performance artist platform built with **Next.js App Router**: music library + immersive playback, video archive, event globe, a booking/contact hub, and an evolving “Studio” area (in rebuild). Optimized for **Vercel** and configured as a **PWA**.
...
- **Routes**
  ...
  - **`/videos`**: Video archive with YouTube embeds + COOP/COEP-safe thumbnails
  - **`/events`**: “World Tour” page with a 3D globe + event list + booking form
  ...
  - **`/studio`**: Placeholder page (“Studio rebuild in progress”)
- **API routes**
