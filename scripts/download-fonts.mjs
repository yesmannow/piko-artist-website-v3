    ...
    https
      .get(cssUrl, options, (response) => {
        ...
        response.on("end", () => {
          ...
          const woff2Matches = data.matchAll(
            /url\(([^)]+\.woff2[^)]*)\)\s+format\(['"]woff2['"]\)/gi,
          );
          ...
          if (matches.length > 0) {
            ...
            if (ttfMatch && ttfMatch[1]) {
              ...
              reject(
                new Error(
                  `No font URL found in CSS for ${cssUrl}. CSS preview: ${data.substring(0, 500)}`,
                ),
              );
            }
          }
...
async function downloadFontVariant(family, weight, style) {
  ...
  try {
    ...
    console.error(
      `[download-fonts] ❌ Failed to download ${family} ${weight}:`,
      error.message,
    );
    throw error;
  }
...
async function downloadFonts() {
  ...
  for (const font of FONTS_TO_DOWNLOAD) {
    for (const weight of font.weights) {
      try {
        const result = await downloadFontVariant(
          font.family,
          weight,
          font.style,
        );
...
if (
  ...
  process.argv[1]?.endsWith("download-fonts.mjs")
) {
  downloadFonts()
    .then((success) => {
