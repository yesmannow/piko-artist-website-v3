...
function checkWorkers() {
  ...
  if (!existsSync(WORKERS_SRC_DIR)) {
    console.log(
      "[check-workers] ✅ No src/workers directory (no workers to check)",
    );
    return true;
  }
  ...
  if (!existsSync(WORKERS_PUBLIC_DIR)) {
    console.error(
      "[check-workers] ❌ public/workers/ directory does not exist",
    );
    console.error("[check-workers] Run: npm run build:workers");
    return false;
  }

  const srcFiles = readdirSync(WORKERS_SRC_DIR).filter((f) =>
    f.endsWith(".worker.ts"),
  );
  const publicFiles = readdirSync(WORKERS_PUBLIC_DIR).filter((f) =>
    f.endsWith(".worker.js"),
  );
  ...
  console.log(
    `[check-workers] ✅ All ${srcFiles.length} worker(s) are compiled`,
  );
  return true;
}
