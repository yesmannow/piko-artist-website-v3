...
const DEFAULT_MODEL_URL = 'https://huggingface.co/timcsy/demucs-web-onnx/resolve/main/htdemucs_embedded.onnx?download=true';
...
function downloadFile(url, filePath) {
  return new Promise((resolve, reject) => {
    ...
    protocol.get(url, (response) => {
      ...
      const totalMB = contentLength > 0 ? (contentLength / 1024 / 1024).toFixed(2) : 'unknown';
      ...
      response.on('data', (chunk) => {
        ...
        if (contentLength > 0) {
          ...
          process.stdout.write(`\r[download-model] Progress: ${percent}% (${(downloaded / 1024 / 1024).toFixed(2)} MB) @ ${speed.toFixed(2)} MB/s`);
        }
      });
...
async function downloadModel() {
  ...
  try {
    ...
    if (result.sizeMB > SIZE_WARNING_MB) {
      console.warn(`[download-model] ⚠️  WARNING: Model file is ${sizeMB} MB (>${SIZE_WARNING_MB} MB)`);
      console.warn(`[download-model] ⚠️  Recommendation:`);
      console.warn(`[download-model] ⚠️    1. Use Git LFS: git lfs track "*.onnx"`);
      console.warn(`[download-model] ⚠️    2. Or use external hosting with NEXT_PUBLIC_MODEL_URL`);
      console.warn(`[download-model] ⚠️    3. Add to .gitignore if not using LFS\n`);
    }
    ...
    console.error(`[download-model] Check your internet connection and the model URL`);
    return false;
  }
}

// Run if called directly
if (import.meta.url === `file://${process.argv[1]}` || process.argv[1]?.endsWith('download-model.mjs')) {
  downloadModel()
    .then((success) => {
